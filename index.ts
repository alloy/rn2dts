...
const SOURCE = "./vendor/react-native-flow-signatures";

const SOURCE_GLOB =
  "{index.js.flow,interface.js.flow,Libraries/*.js.flow,Libraries/**/!(__mocks__|__flowtests__)/*.js.flow}";
const IGNORE_GLOB = [
  "Libraries/{Animated/release,DeprecatedPropTypes,Inspector,JSInspector,NewAppScreen,ReactPrivate,Renderer/implementations,YellowBox}/*.js.flow",
  "Libraries/Animated/src/AnimatedWeb.js.flow"
];
...
rimraf(WORKBENCH, err => {
  ...
  const tsPaths = inputPaths
    .map(inputPath => {
      try {
        const flowCode = captureError(inputPath, "readFileSync", () =>
          fs.readFileSync(path.join(SOURCE, inputPath), "utf-8")
        );
        ...
        const tsPath = path.join(
          WORKBENCH,
          inputPath.replace(/(\.ios)?\.js\.flow$/, ".d.ts")
        );

        let tsCode = captureError(inputPath, "convertFlowToTS", () =>
          convertFlowToTS(flowCode)
        );
        ...
        captureError(inputPath, "mkdirSync", () =>
          fs.mkdirSync(path.dirname(tsPath), { recursive: true })
        );
        captureError(inputPath, "writeFileSync", () =>
